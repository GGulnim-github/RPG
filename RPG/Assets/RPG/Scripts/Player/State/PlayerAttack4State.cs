...
{
    public PlayerAttack4State(StateMachine<PlayerStateName, PlayerController> stateMachine) : base(stateMachine)
    {
    }

    public override void OnEnter()
    {
        Controller.Animator.CrossFadeInFixedTime("WGS_attackA4", 0.1f);
    ...
    {
        if (Controller.Animator.GetCurrentAnimatorStateInfo(0).IsName("WGS_attackA4"))
        {
            ...
            if (animationNormalizedTime > 0.99f)
            {
                ...
                {
                    StateMachine.ChangeState(PlayerStateName.Attack5);
                }
            }
        }
